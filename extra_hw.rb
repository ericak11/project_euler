...
# Problems solved: 1, 3, 4, 5, 6, 7, 9, 10, 13
# Not Solved yet: 2, 8, 11, 12
...
# really_long_num = .to_s.split('').map(&:to_i)
# sum_array = []

# really_long_num.each do |num|
...
# long_nums.each do |num|
#   sum += num
# end

# puts sum #ANSWER 5537376230
...
# sum = 0
# word_array = num_array.map {|num| sum += num.humanize.gsub("-", "").gsub(" ", "").length}
# sum
